...
logger.info('Initializing server (app mode - no auto-launch)...', { port: config.PORT, baseDir: config.BASE_DIR });
...
server.listen(config.PORT, () => {
  logger.info('Server started successfully (app mode)', {
    url: `http://localhost:${config.PORT}`,
    baseDir: config.BASE_DIR,
    mode: 'app'
  });

  // No auto-launcher in app mode - Electron will handle the window
  ...
  // Notify Electron that server is ready (if running in Electron)
  if (process.send) {
